...
### Fixed
- **PDF Stage Timing**: Added `timing: StageTiming::default()` initialization to `OcrResult` within the PDF recognition pipeline.
- **PDF Zero-Copy Processing**: Corrected `Cow::Borrowed(img)` borrowing in multi-page PDF processing closure, eliminating compiler type mismatch and redundant clones.
- **Cross-Feature Compilation**: Fully verified compilation with `--features pdf` and all subtargets.
